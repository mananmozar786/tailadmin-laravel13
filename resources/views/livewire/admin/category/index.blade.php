<div class="space-y-6">
    <!-- Breadcrumb -->
    <x-common.page-breadcrumb :pageTitle="'Category Management'" :activePage="'Categories'" />
    ...
    <!-- Main Card -->
    <div id="category-table-card" class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between lg:p-6 border-b border-gray-100 dark:border-gray-800/50">
            ...
            <div class="flex items-center gap-3">
                <!-- Filters Button -->
                <button 
                    x-on:click="$dispatch('open-drawer', 'category-filters')"
        ...
        <x-ui.table>
            ...
            @forelse($categories as $category)
                <tr class="transition-all hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                    ...
                    <td class="px-5 py-4">
                        @php
                            $statusClasses = match($category->status) {
                        ...
                        <span class="inline-flex items-center rounded-lg px-2.5 py-1 text-xs font-medium {{ $statusClasses }}">
                            <span class="mr-1.5 h-1.5 w-1.5 rounded-full fill-current {{ $category->status === 'active' ? 'bg-green-500' : 'bg-red-500' }}"></span>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            @if($showDeleted)
                                <button wire:click="restore({{ $category->id }})" class="group flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-blue-500 hover:text-blue-500 dark:border-gray-800 dark:text-gray-400 dark:hover:border-blue-500" title="Restore">
                                ...
                                <button wire:click="confirmDeletion({{ $category->id }})" class="group flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-red-500 hover:text-red-500 dark:border-gray-800 dark:text-gray-400 dark:hover:border-red-500" title="Force Delete">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4.5 w-4.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                    </svg>
                                </button>
                            @else
                                <a href="{{ route('admin.categories.edit', $category->id) }}" wire:navigate class="group flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-blue-500 hover:text-blue-500 dark:border-gray-800 dark:text-gray-400 dark:hover:border-blue-500" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4.5 w-4.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                    </svg>
                                </a>
                                <button wire:click="confirmDeletion({{ $category->id }})" class="group flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-red-500 hover:text-red-500 dark:border-gray-800 dark:text-gray-400 dark:hover:border-red-500" title="Delete">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4.5 w-4.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                                    </svg>
                                </button>
        ...
        <div class="px-5 py-4 border-t border-gray-100 dark:border-gray-800">
            {{ $categories->onEachSide(1)->links(data: ['scrollTo' => false]) }}
        </div>
    </div>

    <!-- Delete Confirmation Modal -->
    <x-ui.modal name="confirm-category-deletion" maxWidth="md" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-800 dark:text-white/90">
                Are you sure you want to delete this category?
            </h2>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                {{ $showDeleted ? 'This action will permanently delete the category. This cannot be undone.' : 'The category will be soft-deleted. You can restore it later from the trash.' }}
            </p>

            <div class="mt-6 flex justify-end gap-3">
                <x-ui.button variant="secondary" x-on:click="$dispatch('close-modal', 'confirm-category-deletion')">
                    Cancel
                </x-ui.button>
                <x-ui.button variant="danger" wire:click="delete">
                    {{ $showDeleted ? 'Permanently Delete' : 'Delete Category' }}
                </x-ui.button>
            </div>
        </div>
    </x-ui.modal>
    ...
    <!-- Filter Drawer -->
    <x-ui.drawer name="category-filters" title="Category Filters">
        <div class="space-y-6">
            <!-- Sorting -->
            <div>
            ...
            >
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">Show only deleted categories</p>
                    </div>
                    <button 
                        type="button"
