<div class="space-y-6">
    <!-- Breadcrumb -->
    <x-common.page-breadcrumb :pageTitle="'User Management'" :activePage="'Roles'" />
    ...
    @if (session('error'))
        <div class="flex items-center gap-2 rounded-lg bg-red-50 p-4 text-sm text-red-700 dark:bg-red-500/10 dark:text-red-500">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-4 p-5 md:flex-row md:items-center md:justify-between lg:p-6 border-b border-gray-100 dark:border-gray-800/50">
            ...
            <div class="flex items-center gap-3">
                <!-- Filters Button -->
                <button 
                    x-on:click="$dispatch('open-drawer', 'role-filters')"
        ...
        <x-ui.table>
            <x-slot name="thead">
                <x-ui.table-header label="Role Name" field="name" :sortField="$sortField" :sortDirection="$sortDirection" />
                <x-ui.table-header label="Guard" field="guard_name" :sortField="$sortField" :sortDirection="$sortDirection" />
                <x-ui.table-header label="Permissions" />
                ...
                <tr class="transition-all hover:bg-gray-50/50 dark:hover:bg-white/[0.02]">
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.roles.edit', $role->id) }}" wire:navigate class="group flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-blue-500 hover:text-blue-500 dark:border-gray-800 dark:text-gray-400 dark:hover:border-blue-500" title="Edit">
                            ...
                            <button wire:click="confirmDeletion({{ $role->id }})" wire:loading.attr="disabled" class="group flex h-9 w-9 items-center justify-center rounded-lg border border-gray-200 text-gray-500 transition hover:border-red-500 hover:text-red-500 disabled:opacity-50 dark:border-gray-800 dark:text-gray-400 dark:hover:border-red-500" title="Delete">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4.5 w-4.5">
                ...
                <tr>
                    <td colspan="4" class="px-5 py-10 text-center">
                        <div class="flex flex-col items-center justify-center gap-2">
                             ...
                             <p class="font-medium text-gray-700 dark:text-white/80">No roles found</p>
                        </div>
                    </td>
                </tr>
            @endforelse
    ...
    <x-ui.modal name="confirm-role-deletion" maxWidth="md" focusable>
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-800 dark:text-white/90">
                Are you sure you want to delete this role?
            </h2>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                This will unassign the role from all users. This action cannot be undone.
            </p>

            <div class="mt-6 flex justify-end gap-3">
                <x-ui.button variant="secondary" x-on:click="$dispatch('close-modal', 'confirm-role-deletion')">
                    Cancel
                </x-ui.button>
    ...
    <!-- Filter Drawer -->
    <x-ui.drawer name="role-filters" title="Role Filters">
        <div class="space-y-6">
